...
export default function ProfilePage() {
  ...
  // sync local form fields when store user changes
  useEffect(() => {
    setFullName(user.fullName || "");
    setEmail(user.email || "");
  }, [user.fullName, user.email]);

  // clear local messages when global error changes
  useEffect(() => {
  ...
  async function onFileChange(e) {
    ...
    try {
      ...
      // update avatar via slice (will persist to backend if slice implements it)
      const action = await dispatch(updateAvatar({ avatarUrl: cloudinaryUrl }));
      unwrapResult(action);
      setLocalSuccess("Avatar updated.");
    } catch (err) {
      console.error("Upload/update avatar error:", err);
      setLocalError(err?.message || "Failed to upload/update avatar.");
    } finally {
  ...
  async function onSave() {
    ...
    try {
      ...
      const action = await dispatch(
        updatePersonalInfo({ fullName: fullName.trim(), email: email.trim() })
      );
      unwrapResult(action);
      setIsEditing(false);
  ...
  async function onConfirmDelete() {
    ...
    try {
      ...
      unwrapResult(action);
      // after delete, navigate to login (slice should clear auth/store)
      navigate("/login");
    } catch (err) {
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl shadow-sm p-6">
              <div className="text-center">
                ...
                {localSuccess && (
                  <p className="text-sm text-green-600 mt-3">{localSuccess}</p>
                )}
              </div>
            </div>
          </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-center gap-2">
                      <button
                        ...
                        className="px-6 py-3 bg-red-500 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg hover:bg-red-600 transition-colors font-medium"
                      >
                        Confirm Delete
                      </button>
                      <button
